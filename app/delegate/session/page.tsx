...
function DelegateSessionContent() {
  ...
  if (!committeeId) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50">
      <div className="text-center">
        <h2 className="text-xl font-bold text-slate-800 mb-2">No Committee Context</h2>
        <button onClick={() => router.push('/delegate')} className="text-indigo-600 hover:underline">Return to Dashboard</button>
      </div>
    </div>
  )
  ...
  const renderModule = () => {
    const commonProps = { role: 'delegate' as const, delegates, sessionState, setSessionState: updateSession }
    switch(activeModule) {
      ...
      case 'chat': return <LiveChatModule role="delegate" committeeId={committeeId} />
      case 'archives': return <ArchivesModule committeeId={committeeId} sessionState={sessionState} />
      case 'agenda': return <AgendaModule role="delegate" sessionState={sessionState} />
      case 'gsl': return <GSLModule {...commonProps} />
      case 'voting': return <VotingGridModule {...commonProps} />
      case 'draft': return <DraftResolutionModule role="delegate" sessionState={sessionState} setSessionState={updateSession} committeeId={committeeId} />
      case 'directive': return <DraftResolutionModule role="delegate" sessionState={sessionState} committeeId={committeeId} />
      ...
      case 'coverage': return <LiveChatModule role="delegate" committeeId={committeeId} />
      case 'interviews': return <LiveChatModule role="delegate" committeeId={committeeId} />
      case 'broadcast': return <BroadcastModule role="delegate" committeeId={committeeId} sessionState={sessionState} setSessionState={updateSession} />
