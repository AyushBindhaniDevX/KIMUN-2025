...
export async function POST(req: Request) {
  try {
    ...
    const { history, committee, role, statement, experience, name } = body

    if (!history || !Array.isArray(history)) {
      return NextResponse.json({ success: false, error: 'Missing conversation history' }, { status: 400 })
    }

    const systemPrompt = `You are an expert HR recruiter for the KIMUN Executive Board. You are conducting a fast-track audio interview with an applicant applying for the role of "${role}" in the "${committee}" committee.
    
Here are the applicant's details:
Name: "${name || 'Applicant'}"
Statement of Purpose: "${statement || 'None provided'}"
Prior EB/MUN Experience: "${experience || 'None provided'}"
...
3. For subsequent questions, ask ONE situational or contextual question based on their statement or experience at a time, specifically probing their capability to handle the ${role} role in the ${committee} committee.
...
Respond strictly in the following JSON format:
{
  ...
  "isFinished": false // Set to true ONLY if you are concluding the interview
    ...
    console.error('Error in eb-interview-chat:', error)
    return NextResponse.json(
