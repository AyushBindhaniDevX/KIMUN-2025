...
export default function AIInterviewModal({ isOpen, onClose, application, onComplete }: AIInterviewModalProps) {
  ...
  useEffect(() => {
    if (isOpen && application?.name) {
      const savedStateStr = localStorage.getItem(`eb_ai_interview_${application.name}`);
      if (savedStateStr) {
        try {
  ...
  useEffect(() => {
    if (isOpen && application?.name && (history.length > 0 || step !== 'intro')) {
       localStorage.setItem(`eb_ai_interview_${application.name}`, JSON.stringify({
         step,
         history
       }));
    }
  ...
  const getNextAIResponse = async (currentHistory: {role: 'model' | 'user', text: string}[]) => {
    try {
      setIsAiSpeaking(true);
      setConnectionError(null);
      const res = await fetch('/api/eb-interview-chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          history: currentHistory,
          committee: application?.committeePref1,
          role: application?.rolePref1,
    ...
    try {
      const res = await fetch('/api/evaluate-interview', {
        ...
        body: JSON.stringify({
          ...
          department: `${application?.rolePref1} - ${application?.committeePref1}`
        })
      });

      const data = await res.json();
      if (data.success) {
        localStorage.removeItem(`eb_ai_interview_${application?.name}`);
                 ...
                 {step === 'intro' && (
                    <div className="text-center max-w-lg">
                      ...
                      <p className="text-zinc-300 text-lg mb-8 leading-relaxed">
                        You're about to join a Google Meet style conversational interview. Our AI recruiter will review your application and ask situational questions dynamically based on your EB role preference.
                      </p>
                      {!hasPermissions ? (
            ...
            <div className="w-full md:w-1/3 bg-zinc-900 border-t md:border-t-0 md:border-l border-zinc-800 flex flex-col flex-1 md:h-full z-30">
              ...
              <div className="p-6 border-b border-zinc-800 bg-zinc-900/50">
                ...
                <div className="flex items-center gap-2 text-zinc-400 text-sm mb-3">
                  ...
                  <span>{application?.rolePref1} - {application?.committeePref1}</span>
                </div>
