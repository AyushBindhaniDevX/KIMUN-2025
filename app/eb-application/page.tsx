...
export default function EBApplicationPage() {
  ...
  const handleAIInterviewComplete = async (score: number, feedback: string) => {
    if (!user) return;
    try {
      const appRef = ref(firebaseDb, `eb_applications/${user.uid}`);
      const updates = {
  ...
  const generateContractPDFBytes = async (appData: any): Promise<Uint8Array> => {
    ...
    doc.setFontSize(8.5)
    doc.text("EXECUTIVE BOARD MEMBERSHIP & NON-DISCLOSURE AGREEMENT", 105, 28, { align: 'center' })
  ...
  const handleDownloadContractPDF = async (appData: any) => {
    ...
    try {
      if (appData.contractPdfUrl) {
        ...
        const candidateName = appData.name || user?.displayName || 'Candidate'
        link.download = `KIMUN_EB_Contract_${candidateName.replace(/\s+/g, '_')}.pdf`
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(settingsRef, (snap) => {
      if (snap.exists()) {
        const val = snap.val()
        setAppSettings({ status: val.ebApplicationStatus || 'Open' })
      }
    })
    return () => unsubscribe()
  }, [])
  ...
  const validateStep = (currentStep: number): boolean => {
    ...
    if (currentStep === 2) {
      if (!formData.committeePref1) errors.committeePref1 = "First committee preference is required"
      if (!formData.rolePref1) errors.rolePref1 = "First role preference is required"
      if (!formData.committeePref2) errors.committeePref2 = "Second committee preference is required"
      if (!formData.rolePref2) errors.rolePref2 = "Second role preference is required"
      if (formData.committeePref1 && formData.committeePref2 && formData.committeePref1 === formData.committeePref2 && formData.rolePref1 === formData.rolePref2) {
        errors.committeePref2 = "First and second preferences cannot be exactly the same"
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      rolePref1: formData.rolePref1,
      committeePref2: formData.committeePref2,
      rolePref2: formData.rolePref2,
          ...
          <motion.div variants={fadeIn} className="relative h-80 w-full mb-8 rounded-2xl overflow-hidden border border-slate-200 shadow-lg">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-8 md:p-12 flex flex-col justify-end">
              ...
              <h1 className="text-3xl md:text-5xl font-bold tracking-tight text-white max-w-4xl">
                Join the KIMUN 2026 Executive Board
              </h1>
              <p className="mt-3 text-base md:text-lg text-slate-200 max-w-2xl leading-relaxed">
                Lead and moderate the debates as a key member of the Executive Board.
              </p>
            </div>
          </motion.div>
          ...
          {authLoading || checkingApp ? (
            <div className="bg-white border border-slate-200 rounded-2xl p-16 text-center shadow-sm flex flex-col items-center justify-center">
             ...
             <div className="bg-white border border-slate-200 rounded-2xl p-16 text-center shadow-sm flex flex-col items-center justify-center">
                ...
                <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-slate-900 mb-2">Applications Closed</h2>
                <p className="text-sm text-slate-600">The Executive Board application window has officially closed. Thank you for your interest.</p>
             </div>
          ) : (!application && appSettings.status === 'Paused') ? (
             <div className="bg-white border border-slate-200 rounded-2xl p-16 text-center shadow-sm flex flex-col items-center justify-center">
                <Clock className="h-12 w-12 text-amber-500 mx-auto mb-4" />
                <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-slate-900 mb-2">Applications Paused</h2>
                <p className="text-sm text-slate-600">We are currently processing a high volume of applications. Please check back later.</p>
             </div>
          ) : !user ? (
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-8 md:p-12 shadow-sm text-center">
              ...
              <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-slate-900 mb-3">Apply to KIMUN 2026 Executive Board</h2>
                ...
                {application.status === 'rejected' && (
                  <div className="flex flex-col md:flex-row items-start gap-6 w-full">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-slate-600 mt-2 leading-relaxed">
                        Thank you for your interest and the effort put into applying for the KIMUN 2026 Executive Board. Due to strict slot parameters and high volume, we are unable to accept your application at this time. We encourage you to participate as a delegate or observer in the upcoming conferences.
                      </p>
                    </div>
                  </div>
                )}
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-6 md:p-10 shadow-sm">
              <div className="flex items-center justify-between pb-6 border-b border-slate-100 mb-8">
                <div>
                  <p className="text-[10px] text-indigo-600 font-bold uppercase tracking-widest">Executive Board Onboarding</p>
                ...
                <AnimatePresence mode="wait">
                  ...
                  {step === 2 && (
                    ...
                    >
                      <div className="grid md:grid-cols-2 gap-4">
                        <div>
                          <label className="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1.5">First Choice Committee</label>
                          <select
                            name="committeePref1"
                            value={formData.committeePref1}
                            onChange={handleInputChange}
                            className={`w-full px-4 py-2.5 bg-slate-50 border rounded-lg text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-colors cursor-pointer ${formErrors.committeePref1 ? 'border-red-300 bg-red-50/10' : 'border-slate-200'
                              }`}
                          >
                            <option value="">Select a Committee</option>
                            {committees.map((committee, idx) => (
                              <option key={idx} value={committee.name || committee.id}>{committee.name || committee.id}</option>
                            ))}
                          </select>
                          {formErrors.committeePref1 && (
                            <p className="text-xs text-red-500 mt-1 flex items-center gap-1"><AlertCircle className="h-3.5 w-3.5" /> {formErrors.committeePref1}</p>
                          )}
                        </div>

                        <div>
                          <label className="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1.5">First Choice Role</label>
                          <select
                            name="rolePref1"
                            value={formData.rolePref1}
                            onChange={handleInputChange}
                            className={`w-full px-4 py-2.5 bg-slate-50 border rounded-lg text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-colors cursor-pointer ${formErrors.rolePref1 ? 'border-red-300 bg-red-50/10' : 'border-slate-200'
                              }`}
                          >
                            <option value="">Select a Role</option>
                            {EB_ROLES.map((role, idx) => (
                              <option key={idx} value={role}>{role}</option>
                            ))}
                          </select>
                          {formErrors.rolePref1 && (
                            <p className="text-xs text-red-500 mt-1 flex items-center gap-1"><AlertCircle className="h-3.5 w-3.5" /> {formErrors.rolePref1}</p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="grid md:grid-cols-2 gap-4 mt-4">
                        <div>
                          <label className="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1.5">Second Choice Committee</label>
                          <select
                            name="committeePref2"
                            value={formData.committeePref2}
                            onChange={handleInputChange}
                            className={`w-full px-4 py-2.5 bg-slate-50 border rounded-lg text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-colors cursor-pointer ${formErrors.committeePref2 ? 'border-red-300 bg-red-50/10' : 'border-slate-200'
                              }`}
                          >
                            <option value="">Select a Committee</option>
                            {committees.map((committee, idx) => (
                          ...
                          {formErrors.committeePref2 && (
                            <p className="text-xs text-red-500 mt-1 flex items-center gap-1"><AlertCircle className="h-3.5 w-3.5" /> {formErrors.committeePref2}</p>
                          )}
                        </div>

                        <div>
                          <label className="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1.5">Second Choice Role</label>
                          <select
                            name="rolePref2"
                            value={formData.rolePref2}
                            onChange={handleInputChange}
                            className={`w-full px-4 py-2.5 bg-slate-50 border rounded-lg text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-colors cursor-pointer ${formErrors.rolePref2 ? 'border-red-300 bg-red-50/10' : 'border-slate-200'
                              }`}
                          >
                            <option value="">Select a Role</option>
                            {EB_ROLES.map((role, idx) => (
                          ...
                          {formErrors.rolePref2 && (
                            <p className="text-xs text-red-500 mt-1 flex items-center gap-1"><AlertCircle className="h-3.5 w-3.5" /> {formErrors.rolePref2}</p>
                          )}
                        </div>
                      </div>
                    ...
                    >
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={handleInputChange}
                          placeholder="Why do you wish to join the KIMUN 2026 Executive Board? Highlight what value you can offer to your chosen departments."
